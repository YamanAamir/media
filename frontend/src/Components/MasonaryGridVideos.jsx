    ...
    <div className="columns-1 sm:columns-2 md:columns-3 lg:columns-4 xl:columns-5 gap-4 p-4">
      {images.map((imageItem, index) => (
        <div key={index} className="relative mb-4 break-inside group">
            <video src="https://kota-content.b-cdn.net/app/uploads/2023/10/Kota-loop.mp4" className="hidden"></video>
          <img
            src={imageItem.image}
          ...
          <div className="absolute top-4 p-2 right-4 z-50 cursor-pointer bg-gradient-to-r from-[#5354F4] to-[#2C2DE2] rounded-full hover:border hover:bg-none" onClick={() => setIsPlayed(!isPlayed)}>
        {isPlayed ? <CiPause1 className="text-lg text-white" /> : <CiPlay1  className="text-lg text-white" /> }
      </div>
            <img 
